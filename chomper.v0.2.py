			...
			for column in samfile.pileup(sequence, position-1, position, ignore_overlaps=False, truncate=True):
				...
				for pileupread in column.pileups:
					...
					if pileupread.query_position:
						if debug_counter < 100:
							print("[+] Found a read that overlaps this variant")
						...
						if pileupread.alignment.next_reference_name == sequence:
							...
							if debug_counter < 100:
								print("[+] Mate on different sequence")
	return read_dict
...
if __name__ == '__main__':
	...
	print("[=] Read {} phased variants".format(len(phased_variants)))
	read_names = bam_parser(phased_variants, args['--bam'], args['--sequence'])
	reads_0 = sum(1 for v in read_names.values() if v == set())
	reads_1 = sum(1 for v in read_names.values() if v == {1})
	reads_2 = sum(1 for v in read_names.values() if v == {2})
	reads_12 = sum(1 for v in read_names.values() if v == {1, 2})
	print("[=] Collected information for {} reads".format(len(read_names)))
	print("[=] {} reads with no haplotype info".format(reads_0))
	print("[=] {} reads from haplotype 1".format(reads_1))
	print("[=] {} reads from haplotype 2".format(reads_2))
	print("[=] {} reads with conflict".format(reads_12))
